#import q1 as rosen_2d
import q5 as rosen_2d
import numpy as np
import math
...
def grad_descent(X0, alpha=None, eps=None, method='Gradient descent'):
    ...
    while l2_norm(rosen_2d.rosen_2d(X))>eps:
        k+=1
        alpha = np.sum(grad_x*grad_x)/(np.sum(grad_x * np.matmul(rosen_2d.rosen_2d_Hess(X),grad_x)))
        X=X-alpha*grad_x
        grad_x=rosen_2d.rosen_2d_grad(X)
    return k
