...
def image_deblur(image: np.ndarray, h: np.ndarray, noise_var=5, type='Wiener') -> np.ndarray:
    norm = True
    M, N = image.shape
    H = fft2(h, s=image.shape)
    image = skimage.img_as_float(image)
    if type == 'Wiener':
        u, v = np.indices(image.shape)
        S_w = noise_var ** 2
        nsr = S_w / (S_w * 10 * np.exp(-(np.abs(u - M // 2) - np.abs(v - N // 2)) / 5))
        # nsr = ifftshift(nsr)
        # print(nsr)
        nsr = 0.01
        D = np.conj(H) / (np.abs(H) ** 2 + nsr)
    if type == 'Inverse':
        nsr = 0
        D = 1 / H
        norm = False
    ...
    f_image = fft2(image)  # get fft of image
    im_filtered = ifft2(f_image * D)
    im_filtered = np.abs(im_filtered)

    deblur_im = (im_filtered - np.min(im_filtered)) / (np.max(im_filtered) - np.min(im_filtered)) * 255
    # plt.imshow(deblur_im, cmap = 'gray')
    # plt.show()
